...
internal static class EngineProvider
{
    private static readonly ConcurrentDictionary<string, object>
        BuildLocks = new(StringComparer.OrdinalIgnoreCase);
        ...
        DetectorOptionsBase options)
    {
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            throw new YoloSdkException(
                $"不支持的模型格式：{extension}。SDK 仅支持 .onnx 或 .engine。");
        }

        string sha256 =
            EngineCacheManager.ComputeSha256(modelPath);

        string precision =
            options.EnableFp16 ? "FP16" : "FP32";
