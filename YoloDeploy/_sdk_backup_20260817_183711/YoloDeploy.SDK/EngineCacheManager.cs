...
internal static class EngineCacheManager
{
        ...
        int workspaceMiB)
    {
        ...
        string stem = SanitizeToken(Path.GetFileNameWithoutExtension(onnxPath));
        string gpuToken = SanitizeToken(gpu.Name);
        string hashToken = onnxSha256[..Math.Min(16, onnxSha256.Length)];
        string precisionToken = SanitizeToken(precision.ToLowerInvariant());

        // Same key scheme as the current GitHub WPF EngineCacheManager.
        string cacheKey =
            $"{stem}_{hashToken}_{gpuToken}_"
        ...
        string enginePath = Path.Combine(CacheRoot, cacheKey + ".engine");
        ...
        try
        {
            EngineCacheMetadata? metadata =
                JsonSerializer.Deserialize<EngineCacheMetadata>(
                    File.ReadAllText(descriptor.MetadataPath),
                    JsonOptions);
            ...
            long currentLength = new FileInfo(descriptor.EnginePath).Length;
        ...
        string buildLog)
    {
        ...
        if (!engineFile.Exists || engineFile.Length <= 0)
            throw new YoloSdkException("不能为不存在或为空的 Engine 写入缓存元数据。");
    ...
    {
        string sanitized =
            Regex.Replace(text.Trim(), @"[^A-Za-z0-9._-]+", "_")
                .Trim('_', '.', '-');
