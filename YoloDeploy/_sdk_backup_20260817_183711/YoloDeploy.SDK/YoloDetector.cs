...
/// <summary>
/// Unified Detect / OBB / Seg detector.
/// Task=Auto uses the current Native YoloGetTaskHint API.
/// </summary>
public sealed class YoloDetector : IDisposable
{
        ...
        YoloTask requestedTask)
    {
        ...
        if ((int)requestedTask < (int)YoloTask.Auto ||
            ...
            throw new ArgumentOutOfRangeException(nameof(requestedTask));
        ...
        catch (DllNotFoundException ex)
        {
            throw new YoloSdkException(
                "加载 YoloDeploy.Native.dll 失败。请使用完整 SDK Runtime 目录，不要只复制 YoloDeploy.SDK.dll。",
                ex);
        }
        ...
        if (_handle == IntPtr.Zero)
            throw new YoloSdkException($"加载 TensorRT Engine 失败：{error}");
        ...
        if (detectedTask == YoloTask.Auto)
        {
            ...
            throw new YoloSdkException(
                ...
                + "请确认模型属于当前 GitHub 项目支持的输出布局，并确认 classes.names 数量正确。"
                + Environment.NewLine
                + "ModelInfo:"
                + Environment.NewLine
                + modelInfo);
        ...
        float nms)
    {
        ...
        if (count < 0)
            throw new YoloSdkException($"Detect 推理失败：{error}");

        var detections = new List<DetectBox>(count);

        for (int i = 0; i < count; i++)
        {
            NativeMethods.YoloDetection d = native[i];

            detections.Add(new DetectBox
            {
                ClassId = d.ClassId,
        ...
        float nms)
    {
        ...
        if (count < 0)
            throw new YoloSdkException($"OBB 推理失败：{error}");

        var detections = new List<ObbBox>(count);

        for (int i = 0; i < count; i++)
        {
            NativeMethods.YoloObbDetection d = native[i];

            detections.Add(new ObbBox
            {
                ClassId = d.ClassId,
                ClassName = ResolveClassName(d.ClassId),
                Confidence = d.Score,
        ...
        float maskThreshold)
    {
        ...
        // Native header defines maskStride in UInt16 elements.
        ...
        if (count < 0)
            throw new YoloSdkException($"Seg 推理失败：{error}");

        var detections = new List<SegInstance>(count);

        for (int i = 0; i < count; i++)
        {
            NativeMethods.YoloSegDetection d = native[i];

            detections.Add(new SegInstance
            {
                ClassId = d.ClassId,
                ClassName = ResolveClassName(d.ClassId),
                Confidence = d.Score,
